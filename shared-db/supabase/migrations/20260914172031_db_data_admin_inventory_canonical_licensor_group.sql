-- #2905: canonical licensor grouping and NBCU Submissions purpose in
-- api.db_data_admin_scraped_source_inventory.
--
-- derived-from: 20260914164141
--
-- Every row in all three entity arms now carries one canonical
-- licensor_group_key / licensor_group_name, so the Scraped Properties page shows
-- exactly one Creative and one Submissions section per real licensor instead of
-- grouping by portal-named label text. Disney, Marvel, Pixar, Lucasfilm / Star
-- Wars and 20th Century stay separate licensors. Rows whose licensor is
-- genuinely unresolved or in conflict (OPA scope conflict/unresolved, DCP
-- authority conflict/unresolved, DCP non-authoritative Marvel tag) go into the
-- single 'unresolved' group ("Licensor not yet determined"); they are never
-- dropped and never assigned to Disney. Existing licensor_key and row_key are
-- unchanged, so cursors stay stable.
--
-- plm.nbcu_property rows with source_kind property or franchise_asset were
-- captured from the Product Submissions picker and are now source_purpose
-- Submissions; asset_metadata_label rows stay Creative. The NBCU character and
-- style-guide arms are unchanged. The rest of the function, including the
-- Warner normalized-arm repair from 20260914164141, is carried forward unchanged.
