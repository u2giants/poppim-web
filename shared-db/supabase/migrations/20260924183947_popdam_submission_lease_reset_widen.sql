-- popcre/shared-db#3464; claim #3486.
-- derived-from: 20260923040630
-- Widens the receipt-proven submission-lease reset in two ways:
--   1. Accepts reason not_submitted (worker failed before any provider POST)
--      with NULL p_http_status and a fixed provider_error category.
--   2. Widens provider_definitive_rejection to HTTP 400/401/402/403/422/429.
-- Every other guard is unchanged: current receipt holder only, matching
-- revision, no bound provider job ID, lease still live, no ambiguity markers,
-- never after timeout/disconnect/5xx.
do $prerequisites$
declare
...
begin
  ...
  -- Two accepted reasons, each with its own evidence contract.
  --
  -- not_submitted: the worker failed before any provider POST, so there is no
  --   HTTP response. Requires NULL p_http_status and p_provider_error carrying
  --   the fixed reason category {"category":"not_submitted"}. This is a
  --   caller-supplied statement of the worker's own local failure, not a claim
  --   about an external HTTP response.
  --
  -- provider_definitive_rejection: the provider answered the POST with a
  --   parsed definitive 4xx rejection. Requires p_http_status in the widened
  --   set 400/401/402/403/422/429 and a nonempty JSON object p_provider_error.
  --   The caller must authenticate the provider response; SQL cannot
  --   distinguish a provider response from a proxy response based on status
  --   and body alone.
  --
  -- Every other reason stays refused. Timeout, disconnect and 5xx remain on
  -- the existing ambiguous-submission path.
  if p_reason = 'not_submitted' then
    if p_http_status is not null
       or jsonb_typeof(p_provider_error) is distinct from 'object'
       or p_provider_error ->> 'category' is distinct from 'not_submitted' then
      raise exception 'reset_bulk_operation_submission_lease: not_submitted requires NULL http status and fixed provider_error category'
        using errcode = '22023';
    end if;
  elsif p_reason = 'provider_definitive_rejection' then
    if p_http_status is null
       or p_http_status <> all(array[400, 401, 402, 403, 422, 429])
       or jsonb_typeof(p_provider_error) is distinct from 'object'
    ...
    raise exception 'reset_bulk_operation_submission_lease: reason must be not_submitted or provider_definitive_rejection'
      using errcode = '22023';
  end if;
  ...
  v_reset_job := (v_job - 'submission_owner' - 'lease_expires_at'
                 ...
                 || jsonb_build_object(
                      ...
                      'last_definitive_rejection_reason', p_reason,
...
comment on function public.reset_bulk_operation_submission_lease(text, bigint, text, text, text, integer, jsonb) is
  'For popcre/shared-db#3464 (widened from #3418). Consumes only the current, unexpired submission receipt on an unbound running operation. Accepts two reasons: not_submitted (NULL http status, fixed provider_error category {"category":"not_submitted"}) when the worker failed before any provider POST, and provider_definitive_rejection (HTTP 400/401/402/403/422/429 with nonempty JSON error body) after PopDAM presents parsed provider-origin definitive rejection evidence. The caller must authenticate the provider response; SQL cannot distinguish provider from proxy origin. Timeout, disconnect, all other 4xx, 5xx, expired or ambiguous lease, stale revision, wrong holder or receipt, and bound provider job cannot reset. The function never returns or remints a receipt.';
