-- #3174: Coca-Cola Submissions, WWE Creative, Peanuts Creative and Sesame Workshop
-- Creative / Submissions sections in api.db_data_admin_scraped_source_inventory.
--
-- derived-from: 20260917005650
--
-- New property arms (all other arms, row keys, grants and the character and
-- style-guide branches are carried forward unchanged from 20260917005650):
--   coca-cola-submissions  distinct brand values used by approval routes in the
--                          latest complete plm.coke_capture
--                          (plm.coke_approval_vocabulary_value, kind 'brand').
--   wwe-creative           plm.wwe_creative_brand, latest complete creative capture.
--   peanuts-creative       plm.peanuts_art_program, latest complete peanuts capture.
--                          Identity is the ledger member identity
--                          peanuts_tenovos|source.tenovos_art_program|source_value_id
--                          so existing decisions attach.
--   sesame-creative        plm.sesame_brand, latest complete sesame capture, one row
--                          per exact label. Identity is the ledger member identity
--                          sesame_thelettera_netx|source.thelettera_brand|label:<label>.
--   sesame-submissions     new landing table plm.sesame_submission_property_option
--                          (MediaBox property options). Rows arrive from the
--                          source-data loader; the table starts empty.

create table plm.sesame_submission_property_option (
  ...
  constraint sesame_submission_property_option_pkey primary key (option_key),
  constraint sesame_submission_property_option_key_nonblank_chk check (btrim(option_key) <> ''),
  constraint sesame_submission_property_option_label_nonblank_chk check (btrim(exact_label) <> ''),
  constraint sesame_submission_property_option_ordinal_chk check (ordinal >= 0),
  constraint sesame_submission_property_option_field_nonblank_chk check (btrim(source_field) <> ''),
  constraint sesame_submission_property_option_raw_obj_chk check (jsonb_typeof(raw) = 'object')
);

comment on table plm.sesame_submission_property_option is
  'LICENSED SESAME WORKSHOP SOURCE EVIDENCE, NOT CANONICAL MASTER DATA. The Property '
  'choices offered by the Sesame Workshop Submissions (MediaBox) portal, verbatim. '
...
-- Self-check: read each new section's source against this database and confirm the
-- function body carries each new section. Counts are reported, not asserted,
-- because preview databases may hold no licensor captures.
do $selfcheck$
declare
  v_def text := pg_get_functiondef('api.db_data_admin_scraped_source_inventory(text,text,text,integer)'::regprocedure);
  v_key text;
  v_n bigint;
begin
  ...
  select count(distinct v.value_key) into v_n
  from plm.coke_approval_vocabulary_value v
  where v.vocabulary_kind = 'brand'
    and v.capture_id = (select c.id from plm.coke_capture c where c.status = 'complete'
      order by c.source_captured_at desc, c.load_completed_at desc, c.id desc limit 1);
  raise notice '#3174 self-check: Coca-Cola Submissions rows %', v_n;

  select count(*) into v_n from plm.wwe_creative_brand b
  where b.capture_id = (select c.id from plm.wwe_creative_capture c where c.status = 'complete'
    order by c.source_captured_at desc, c.load_completed_at desc, c.id desc limit 1);
  raise notice '#3174 self-check: WWE Creative rows %', v_n;

  select count(*) into v_n from plm.peanuts_art_program a
  ...
  where b.capture_id = (select c.id from plm.sesame_capture c where c.status = 'complete'
    order by c.source_captured_at desc, c.load_completed_at desc, c.id desc limit 1);
  raise notice '#3174 self-check: Sesame Workshop Creative rows %', v_n;

  select count(*) into v_n from plm.sesame_submission_property_option;
  raise notice '#3174 self-check: Sesame Workshop Submissions rows %', v_n;

  if not (select relrowsecurity from pg_class
          where oid = 'plm.sesame_submission_property_option'::regclass) then
    raise exception '#3174 self-check: RLS not enabled on plm.sesame_submission_property_option';
  end if;
  if has_table_privilege('authenticated', 'plm.sesame_submission_property_option', 'select')
     or has_table_privilege('anon', 'plm.sesame_submission_property_option', 'select') then
    raise exception '#3174 self-check: client roles can read plm.sesame_submission_property_option';
  end if;
end
$selfcheck$;
