...
| B1. Ship the self-service additive-lane boundary classifier (reusing existing lexers) + dirty-first tests | ✅ done 2026-09-18 | 2026-09-17 | Merged in PR #3228 (`c8dd3baa`): classifier over the production tokenizer (two output views, no third parser), 20 dirty-first fixtures + lexing pins. |
| B2. Admit route `self-service-additive` in `QUEUE_ROUTES`/`ROUTES_BY_WORK_TYPE`/`evaluateAdmission`/`renewalIssueScope`/expand paths; excluded from orchestrator refill — ONLY in the same pull request as B3 (or after it) | ✅ done 2026-09-18 | 2026-09-17 | Merged in PR #3228: route admitted across the lane machinery and admission, own audit section, never refills; AGENTS.md banner/§0.0-C/§4 in the same change; sync test green on main. |
| B3. Classifier enforced at merge time (pre-lock step in the guarded merge workflow) + write down the already-existing merge-dispatch right — must land WITH or BEFORE B2 | ✅ done 2026-09-18 | 2026-09-17 | Merged in PR #3228: pre-lock invocation in the guarded merge (route-gated), ordering rule made permanent by the sync test, merge-dispatch right recorded in the workflow header. |
| B4. Document the lane (AGENTS.md front-matter banner + §0.0-C/§4, both `shared-db-change` skills, memory) incl. the author's exact commands | ✅ done 2026-09-18 | 2026-09-17 | AGENTS.md landed with B2/B3 (PR #3228); both machine-local skill copies (`~/.zcode/skills/shared-db-change`, `~/.agents/skills/codex-shared-db-change`) carry the author command sequence, and the codex memory entry is written (2026-09-17). |
| C1. Write the merge→production hop table for the #2758 ephemeral route | ✅ done 2026-09-18 | 2026-09-17 | Merged in PR #3228: `docs/agents/ephemeral-route-hop-table.md`, seven hops each naming actor and why manual. |
