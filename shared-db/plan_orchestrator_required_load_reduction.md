...
|---|---|---|---|
| A1. Label every open unlabelled issue (scope block verified first) | ✅ done 2026-09-17 | 2026-09-17 | Re-derived list was #3212, #3191, #3174, #3149, #3148, #3125, #2530; scope blocks drafted for #3191/#3149/#3148 (noted in each body), then `db-work` applied to all seven. Audit rerun prints no `UNLABELLED ISSUES` block. |
| A2. Close out expired author-lease claim #3089 via `--release-claim` (PR already merged) | ✅ done 2026-09-17 | 2026-09-17 | `--release-claim 3089 --owner claude-opus-5-3027-step6 --confirm-finished` exited 0; audit rerun prints no `EXPIRED AUTHOR LEASES` section at all. |
| A3. Ship read-only scheduled `Queue Hygiene Report` (write-stripped subcommand + `author-lane-abandonment-audit.yml` shape) | ✅ done 2026-09-17 | 2026-09-17 | Merged as PR #3216 (merge `13efcd77`; Grok 4.6 REVISE finding fixed — two more refused hooks + a completeness guard classifying every githubIo hook — Muse Spark 1.3 APPROVE at head `e9acf0fe`, 7 tests): standalone `scripts/queue-hygiene-report.mjs` (imports the audit machinery; moved off the protected lane manager when PR #3215 held that lane), daily read-only `queue-hygiene-report.yml`, tools-offline wiring, truth-audit site 308. Gate PASSED: workflow_dispatch run 35268440957 green on the live backed-up queue. |
| B1. Ship the self-service additive-lane boundary classifier (reusing existing lexers) + dirty-first tests | 🟡 in review 2026-09-17 | 2026-09-17 | `scripts/check-self-service-additive-lane.mjs` consumes the production tokenizer (#2758) in two views — folded (ALLOWLIST shapes, fullmatch semantics, boundary-qualified, security-invoker required) and keep_dollar_quoted (routine-body reference scanning) — via `sql_top_level_statements`' new `spans`/`keep_dollar_quoted` parameters; 20 dirty-first tests plus `scripts/test_self_service_boundary_lexing.py` pinning the tokenizer views; both wired into CI. |
| B2. Admit route `self-service-additive` in `QUEUE_ROUTES`/`ROUTES_BY_WORK_TYPE`/`evaluateAdmission`/`renewalIssueScope`/expand paths; excluded from orchestrator refill — ONLY in the same pull request as B3 (or after it) | 🟡 in review 2026-09-17 | 2026-09-17 | Route in `QUEUE_ROUTES`, `ROUTES_BY_WORK_TYPE.structural`, `STRUCTURAL_ROUTES`; all five hard-coded route sites now test `STRUCTURAL_ROUTES.includes`; `buildDynamicQueues` collects a `selfServiceLane` bucket that never refills; audit prints its own section; AGENTS.md §0.0-C route paragraph + §4 sentence + banner exception in the same change; `check-lane-route-sync.test.mjs` guards the agreement. |
| B3. Classifier enforced at merge time (pre-lock step in the guarded merge workflow) + write down the already-existing merge-dispatch right — must land WITH or BEFORE B2 | 🟡 in review 2026-09-17 | 2026-09-17 | `check-self-service-additive-lane.mjs --pr <n>` invoked in `guarded-migration-merge.yml` before the quota check and merge lane (route-gated); the sync test refuses any tree that admits the route without the merge-time step (PR #3204 finding 3 made permanent); the §8 merge-dispatch right recorded in the workflow header. |
| B4. Document the lane (AGENTS.md front-matter banner + §0.0-C/§4, both `shared-db-change` skills, memory) incl. the author's exact commands | 🟡 in review 2026-09-17 | 2026-09-17 | AGENTS.md banner, §0.0-C route paragraph and §4 sentence land with B2/B3. Remaining after merge: both skill copies and the memory entry. |
| C1. Write the merge→production hop table for the #2758 ephemeral route | 🟡 in review 2026-09-17 | 2026-09-17 | [`docs/agents/ephemeral-route-hop-table.md`](docs/agents/ephemeral-route-hop-table.md): seven hops, each with actor, evidence and why manual/automatic; records that the automatic-promotion job already covers the preview-triggered chain (incl. the #3039 ephemeral substitution). |
