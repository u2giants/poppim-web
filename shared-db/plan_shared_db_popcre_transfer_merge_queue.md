...
|---|---|---|---|
| 0. Obtain the exact owner authorization and reserve a change window | ✅ done | 2026-09-18 | Albert: "approved, go ahead with the transfer" (#2530 comment 5720258785). Freeze announced on #2530 (5723900929). |
| 1. Re-capture the live transfer baseline and prove a safe stop point | ✅ done | 2026-09-17 | PR #3120, merge `f3bfe4f7`. Artifact `docs/verification/shared-db-popcre-transfer-preflight-20260917T000343Z.json` reports `readyToTransfer: false`. Its blockers are window-time, not code: Step 0 owner authorization comment not recorded; destination (`popcre`) Actions policy unreadable without org-admin rights (HTTP 403); orchestrator not confirmed quiescent (marker #3106); author-acquisition mutation lane held; Actions runs active. Re-capture at the window. |
| 2. Land transfer-compatible repository identity handling | ✅ done | 2026-09-17 | PR #3121, merge `629657fa`. Follow-up pre-transfer gaps (operator-trust association and outcome-evidence repository restriction) closed in the #2530 gap PR. The historical authorization prerequisite was satisfied in Step 0 on 2026-09-18. |
| 3. Freeze repository movement and create the final recovery pack | ✅ done | 2026-09-18 | Baseline `docs/verification/shared-db-popcre-transfer-preflight-20260918T090454Z.json`: only blockers were 17 zero-job, uncancellable queued runs for merged PR #3246's head `5d6d088a`, waived by Albert in his own words ("ignore those 17 stuck runs and transfer now"). Source ID 1275568548, main `1886fb38`, ref digest and verified bundle sha256 on #2530 (5727769182). |
| 4. Transfer `u2giants/shared-db` to `popcre/shared-db` | ✅ done | 2026-09-18 | HTTP 202; `popcre/shared-db` returns ID 1275568548, public, default `main` at `1886fb38`; every ref identical to the pre-transfer mirror; old web URL 301-redirects, old API and Git URLs resolve to the new repository. |
| 5. Reconcile access, settings, integrations, and secrets | ✅ done | 2026-09-18 | `docs/verification/shared-db-popcre-transfer-postcheck-20260918T090657Z.json`: inventory identical to pre-transfer except owner/ownerType and `devopswithkube` reading `admin` (inherited from that account's existing popcre organization admin role; a repository-level grant cannot lower it). No repair needed. Secret names, variables, environments, hooks, protection and 11 required contexts unchanged; credential proof = this step's identity PR checks and the post-merge consumer sync. |
| 6. Land canonical identity and external-reference updates | ✅ done | 2026-09-18 | Identity PR #3254 merged as `2f204a48` (closes #3253); post-transfer fixtures PR #3256 merged as `08f9a83a` (closes #3255). Guarded-lane merges start no push workflows (#3264), so the consumer sync ran on `f9809546` (#3263): run 35351543065 green for all nine consumers, each `shared-db/AGENTS.md` naming `popcre/shared-db` (#2530 comment 5730865894). Cross-repository skill references: popcre/ai-devops#589, u2giants/licensor-source-data#82. |
