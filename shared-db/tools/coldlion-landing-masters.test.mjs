...
test("generated SQL is a re-runnable upsert, reconciles cleared slots, and never uses history sealing",()=>{
  ...
  assert.match(sql,/on conflict \(company_code, customer_code\) do update/i); assert.match(sql,/source_raw jsonb/i); assert.match(sql,/select distinct on \(cl\.natural_key\) cl\.natural_key, cl\.new_raw/i); assert.match(sql,/coalesce\(prior\.new_raw, \(to_jsonb\(t\)/i); assert.doesNotMatch(sql,/select cl\.new_raw[\s\S]*order by cl\.changed_at desc limit 1/i); assert.match(sql,/case when t\.company_code is null then 'inserted'/i); assert.match(sql,/extensions\.digest/i); assert.match(sql,/absent from current source snapshot/i); assert.match(sql,/delete from coldlion\.item_merch_group/i); assert.match(sql,/not exists \(select 1 from _stage_item_merch_group/i); assert.doesNotMatch(sql,/truncate|window_ledger|history_page_ledger/i); assert.equal((sql.match(/\bbegin;/gi)??[]).length,1); assert.equal((sql.match(/\bcommit;/gi)??[]).length,1);
});
