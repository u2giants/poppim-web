...
- `pickReviewer(sequence, io)` calls `findBusyReviewers`; on an unreadable scan it deliberately falls back to ordinary rotation so it does not silently send all work to the paid overflow reviewer. Preserve that business intent while making quota/budget failures fail before locking rather than masquerade as availability. (Superseded 2026-09-16, issue #3130: `pickReviewer` no longer reads busy state at all.)
