...
> **Superseded in part 2026-09-16 (issue #3130, owner ruling "remove all limits on using one reviewer simultaneously").** Every per-provider serialization, alias serialization, all-busy `review-wait`, waker, wait-claim and wait/recovery lock described in this step is retired and must not be rebuilt. What remains is the per-exact-review reservation, overflow ordering, engine exclusion and the preflight usable gate. The Verification gate below is satisfied by `scripts/orchestrator-flow/reviewer-allocation.test.mjs` as of #3130, not by the deleted wait tests. **Superseded 2026-09-16 (issue #3130):** no per-provider serialization, no wait, waker or wait-claim; one provider runs any number of exact reviews, each on its own reservation `refs/db-reviewer-reservations/<provider>/<wrapper>/<issue>-<pr>-<head>-<bundle16>`.
...
- Reviewer supply tests: all six active reviewers busy creates a durable ordered `review-wait`, never a duplicate; release and independent reconciliation each wake exactly one compatible waiter through the allocator; a dead releaser strands nothing; cap 8 does not imply eight simultaneous reviewers. Superseded 2026-09-16 (issue #3130, owner ruling "remove all limits on using one reviewer simultaneously"): there is no busy state, no `review-wait` and no wait queue; each exact review holds its own reservation.
