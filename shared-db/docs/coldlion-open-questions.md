...
| 2.36 | 🔴 **OPEN AND BLOCKING A SCHEMA DECISION — what identifies a `/proddetails` row, and what do same-`prodLineSeq` groups mean? SENT 2026-09-20.** A full read-only scan of the `/proddetails` population (every `prodOrderNo` 1–60000, EDGEHOME, the only company code; **3,819 orders return rows**) found **11 groups across 9 orders** where two or more rows share one `prodOrderNo` + `prodLineSeq`. `pkey` is distinct in every group and unique across the population. The groups take **five distinct shapes**: an exact duplicate in every business field created 140 ms apart (20959/1); a quantity split across two rows of one item (23465/1, 23465/2, 21918/3, 21928/7); one item on one line at many different costs (23475/1 — seven rows, 23475/3 — four rows, 23475/6); two different `itemNo` on one line (21907/22, 23587/1); and a zero-quantity row with a different `itemPkey` shadowing a real one (20344/10). **We never asked about this endpoint.** The `prodLineSeq` answer of 2026-08-17 was about `/prodHistory`, where identity is `(prodOrderNo, prodLineSeq, prepackItemNo)` — the line number alone was never asserted as a row identity. Asked: what identifies a row here; what `prodLineSeq` means on this endpoint; for each shape whether to keep separate, sum, or drop; whether a status/cancellation/revision flag exists that the endpoint does not return; and whether any of this changes what we owe the factory. **Until answered, do not take the constraint decision on #3234** — dropping the unique constraint would land the exact-duplicate and split-quantity shapes as separate lines, and if either should have been summed the landed cost for that order is wrong. The 9 orders are held out of the load; nothing is recorded incorrectly. | JamieLynn | live read-only scan 2026-09-20; full question text and worked examples in popcre/shared-db#3234 (comment 2026-09-20); blocker ticket #3351 | **SENT 2026-09-20 by Albert. Awaiting reply — do not infer an answer and do not re-ask.** |
