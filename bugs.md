...
**Implementation plan:** [`plan_jev_activity_triage.md`](plan_jev_activity_triage.md) — read its STATUS table first; GitHub issue #9 owns the open pilot.
...
Jev is worth a small shadow-mode pilot for one bounded classification task. The final implementation scope is narrower than this initial survey: read only a newly posted product comment and classify it as routine update, blocker/dependency, decision, follow-up reminder, or unclear. Urgency, revisions, approvals, imported updates, and every other candidate below are explicitly deferred; do not add them during this pilot. Staff must confirm before any record is created or changed.
...
The final plan rejects a browser call and the hosted Supabase Edge Function because the latter receives an overly broad project credential. It uses a separately deployed, least-privilege Deno gateway with no service-role/database/JWT-signing credential. That boundary verifies the signed-in user, limits which fields leave Supabase, enforces phase-specific time/token/spend limits, validates the returned shape, and fails without blocking the normal workflow. Persistent AI fields or a new database contract remain separately governed work in canonical `shared-db`; this Poppim session does not orchestrate it.

## HIGH — the existing deployment control-plane token uses plaintext HTTP

Evidence: `.github/workflows/deploy.yml:29`, `.github/workflows/deploy.yml:110-118`
Confidence: high

The current workflow sends the Coolify bearer token to a literal `http://` control-plane address. A network attacker could steal the token or alter deployment operations. Phase 0 and Step 1.6 of the implementation plan block every other non-prose change until a separately authorized infrastructure change provides certificate-valid HTTPS or an authenticated encrypted private tunnel, and the Step 6 repository tests prove no Authorization header is sent over plaintext or to a wrong host/certificate. Do not reuse or copy the current transport.
...
Comments and imported updates are unstructured text. Staff currently retype the same information separately as dependencies, decisions, or reminders. The approved first-pilot design classifies a newly posted human comment as one of:
...
## DEFERRED — Control Room attention triage can use semantic signals
...
## DEFERRED — revision and submission intake can be routed into fixed buckets
...
## DEFERRED — product category is a safe evaluation task, but low business value
...
## DEFERRED — stage or workflow-template suggestions are possible but riskier
...
1. Build the plan's audited, blinded historical-comment corpus with exactly the five final labels. Imported updates and the deferred classes stay out. Real text remains blocked until the owner privacy/reviewer gates and shared contract are complete.
...
4. Complete vendor/security and contract review first. The current Master Customer Agreement says customer data is not used to train model weights without prior consent, but grants TypeSafe perpetual rights to process Customer Data to derive telemetry and permits telemetry use without restriction. Albert must explicitly accept or reject that license, together with retention/deletion, DPA, subprocessor, and United States processing terms, before any real comment is sent.
5. Only after the exact-label offline gate passes, add an optional suggestion beside a newly posted comment. Never block comment posting; never auto-create or auto-update a business record; preserve the normal app when Jev is unavailable.
