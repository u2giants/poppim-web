...
- **Pipeline:** `src/features/pipeline/api.ts` fetches `stage`, `product`, and related lookup context. `PipelinePage` groups products by stage into droppable columns; `@dnd-kit` drag -> `setProductStage` (optimistic, reverts on error). The Licensed pipeline mirrors the ClickUp `Licensing Management` board by querying top-level open/custom ClickUp tasks (`clickup_parent_id` null, `clickup_status_type` open/custom) sorted by `clickup_updated_at` descending.
