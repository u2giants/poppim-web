...
export async function fetchDesigns(opts: FetchDesignOpts = {}): Promise<Design[]> {
  const { data, error } = await pim().from('design').select('*').order('status').order('title').limit(opts.limit ?? 300)
...
export async function fetchDesignCollections(opts: FetchDesignOpts = {}): Promise<DesignCollection[]> {
  const { data, error } = await pim().from('design_collection').select('*').order('updated_at', { ascending: false }).limit(opts.limit ?? 300)
  const q = opts.search?.trim().toLowerCase()
  return unwrap<any[]>({ data, error })
...
export async function fetchProductCountsByDesign(): Promise<Map<string, number>> {
  const { data, error } = await pim().from('product').select('design_id')
...
export async function fetchProjectCountsByDesignCollection(): Promise<Map<string, number>> {
  const { data, error } = await pim().from('project').select('design_collection_id')
  const counts = new Map<string, number>()
