...
export async function enrichProductRowsWithBoardFields<T extends ProductLikeRow>(rows: T[]): Promise<Array<T & Record<string, unknown>>> {
  ...
  for (let i = 0; i < ids.length; i += BOARD_ENRICH_BATCH_SIZE) {
    batches.push(ids.slice(i, i + BOARD_ENRICH_BATCH_SIZE))
  }
  // Batches are independent reads; running them serially made a 500-row page
  // cost five sequential round trips.
  const results = await Promise.all(batches.map((batch) =>
    asDynamic(api()).from('pm_product_board').select('*').in('id', batch)))
